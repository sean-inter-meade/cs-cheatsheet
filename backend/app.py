...
app = FastAPI(title="CS Cheatsheet", version="1.0.0")
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        timestamps = [t for t in self._clients.get(client_ip, []) if t > cutoff]
...
async def on_startup() -> None:
    ...
    if not INTERCOM_API_TOKEN:
        logger.error("INTERCOM_API_TOKEN is not set. Canvas requests will fail.")
    else:
