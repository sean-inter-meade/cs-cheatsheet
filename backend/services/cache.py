...
from backend.models.cheatsheet import CheatsheetData
...
class CheatsheetCache:
    def __init__(self, ttl: int | None = None, max_size: int | None = None) -> None:
        ...
        self._store: dict[str, tuple[float, CheatsheetData]] = {}

    def get(self, conversation_id: str) -> Optional[CheatsheetData]:
    ...
    def put(self, conversation_id: str, data: CheatsheetData) -> None:
        if conversation_id not in self._store and len(self._store) >= self._max_size:
            oldest_key = min(self._store, key=lambda k: self._store[k][0])
            del self._store[oldest_key]
        self._store[conversation_id] = (time.time() + self._ttl, data)
